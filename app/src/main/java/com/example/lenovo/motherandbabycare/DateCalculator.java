...
public class DateCalculator {
    ...
    public static DateCalculator calculateAge(Calendar startDate, Calendar endDate) {
        Calendar mycalender1 = (Calendar) startDate.clone();
        Calendar mycalender2 = (Calendar) endDate.clone();

        long d1 = mycalender1.getTimeInMillis();
        long d2 = mycalender2.getTimeInMillis();

        if (d1 > d2) {
            mycalender1 = (Calendar) endDate.clone();
            mycalender2 = (Calendar) startDate.clone();
        }

        int mDay = mycalender1.get(Calendar.DAY_OF_MONTH);
        int mMonth = mycalender1.get(Calendar.MONTH);
        int mYear = mycalender1.get(Calendar.YEAR);

        int tDay = mycalender2.get(Calendar.DAY_OF_MONTH);
        int tMonth = mycalender2.get(Calendar.MONTH);
        int tYear = mycalender2.get(Calendar.YEAR);
